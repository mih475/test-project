...
def simulate_fixed_tp(d,tr,tp_r):
    entry=float(tr.entry_price); risk=float(tr.risk_pts); direction=int(tr.dir_sign); stop=float(tr.stop_price)
    ...
    for j in range(loc,len(d)):
        r=d.iloc[j]
        if direction==1: hit_s=r.low<=stop; hit_t=r.high>=target
        else: hit_s=r.high>=stop; hit_t=r.low<=target
        if hit_s and hit_t:return -1.0,'ambiguous_stop_first'
        if hit_s:return -1.0,'stop'
        if hit_t:return float(tp_r),'tp'
    px=float(d.iloc[-1].close); return direction*(px-entry)/risk,'eod'

def stats(vals):
    v=pd.Series(vals,dtype=float).dropna(); w=v[v>0]; l=v[v<=0]
    pf=w.sum()/abs(l.sum()) if len(l) and abs(l.sum()) else np.inf
    eq=v.cumsum(); return len(v),100*(v>0).mean(),v.mean(),pf,v.sum(),(eq-eq.cummax()).min()
...
def report(label,g,col):
    ...
    return {'exit':label,'n':raw[0],'raw_win':raw[1],'raw_avgR':raw[2],'raw_PF':raw[3],'raw_totalR':raw[4],'raw_DD':raw[5],'ES1_win':es[1],'ES1_avgR':es[2],'ES1_PF':es[3],'ES1_DD':es[5],'MES1_win':mes[1],'MES1_avgR':mes[2],'MES1_PF':mes[3],'MES1_DD':mes[5]}
...
def main():
    ...
    for tp in TP_LEVELS:
        ...
        for _,tr in t.iterrows():
            d=by.get(pd.Timestamp(tr.session)); rr,_=simulate_fixed_tp(d,tr,tp) if d is not None else (np.nan,'missing_session'); vals.append(rr)
    ...
    for tp in TP_LEVELS:
        ...
        for per,mask in periods.items(): x=report(f'TP_{tp:g}R',t[mask],col); x['period']=per; rows.append(x)
    ...
    for tp in TP_LEVELS:
        ...
        piv.append({'tp_R':tp,'dev_win':a.raw_win,'val24_win':b.raw_win,'dev_ES1_avgR':a.ES1_avgR,'val24_ES1_avgR':b.ES1_avgR,'dev_ES1_PF':a.ES1_PF,'val24_ES1_PF':b.ES1_PF,'floor_avgR':min(a.ES1_avgR,b.ES1_avgR),'floor_PF':min(a.ES1_PF,b.ES1_PF)})
    ...
    if len(qualified):
        chosen=float(qualified.iloc[0].tp_R); print('FROZEN_CANDIDATE_TP_R',chosen); print('\nOUT-OF-SAMPLE REVEAL')
    ...
    else: print('FROZEN_CANDIDATE_TP_R NONE')
    print('\n70_PERCENT_CHECK')
    for tp in TP_LEVELS:
        a=out[(out.exit==f'TP_{tp:g}R')&(out.period=='DEV21-23')].iloc[0]; b=out[(out.exit==f'TP_{tp:g}R')&(out.period=='VAL24')].iloc[0]
